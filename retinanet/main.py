...
from retinanet import infer, train, utils, infer_video
...
def parse(args):
    ...
    parser_train.add_argument('--warmup', metavar='iterations', help='numer of warmup iterations', type=int, default=128)
    parser_train.add_argument('--gamma', metavar='value', type=float, help='multiplicative factor of learning rate decay', default=0.01)
    ...
    parser_train.add_argument('--val-iters', metavar='number', type=int, help='number of iterations between each validation', default=5000)
...
def load_model(args, verbose=False):
    ...
    # print('gonna load model...  ', args.backbone, args.classes)
...
def worker(rank, args, world, model, state):
    ...
    if args.command == 'train':
        ...
        if model is None:
            print('Loading CUDA engine from {}...'.format(os.path.basename(args.model)))
